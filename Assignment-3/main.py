...
RNN = myRNN(vocab_size=256, hidden_size=p.hidden, temperature=p.temperature)

# get the text
all_text = list(open(p.text_src, 'r').read())
...
while True:
    ...
    if epoch % p.checkpoint_interval == 0 and pointer == 0:
        gen_text = RNN.get(seed=inputs[0], hidden_state=hidden_state, n=100)
